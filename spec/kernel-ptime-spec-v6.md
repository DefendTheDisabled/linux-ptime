...
Edited by: Principal-Opus 2026-04-07 (v6: file_setattr API migration)
Reviewed by: Associate-Opus 2026-04-07 (code audit: 3 critical found, all fixed)
Status: v6 — file_setattr write API. Native/mapped architecture. Rename-over on all 5 FS.
...
- **v6**: **Userspace write API migrated from utimensat to file_setattr.**
  - **AT_UTIME_PTIME removed entirely.** No more utimensat extension, times[2] hack, or vfs_utimes flags parameter.
  - **file_setattr (syscall 469)** replaces utimensat as the write path. Uses `struct file_attr` with VER1 extension (40 bytes): `fa_ptime_sec` (__s64) + `fa_ptime_nsec` (__u32) + `fa_ptime_pad` (__u32) appended after VER0 fields.
  - **Size versioning**: `copy_struct_from_user()`/`copy_struct_to_user()` — same pattern as clone_args, mount_attr. Forward/backward compatible. `FILE_ATTR_SIZE_VER1 = FILE_ATTR_SIZE_VER0 + 16 = 40`.
  - **Dual dispatch in file_setattr**: ptime → `notify_change()` (iattr/setattr path, works on ALL 5 FS); xflags → `vfs_fileattr_set()` (existing path). Architecture: `.fileattr_set` callbacks handle flags only; ntfs3/FAT/exFAT lack them entirely. ptime reuses the per-FS `.setattr` handlers unchanged.
  - **Validation**: `fa_ptime_nsec >= NSEC_PER_SEC` → EINVAL. `fa_ptime_pad != 0` → EINVAL (forward compat).
  - **statx remains sole read path.** file_getattr does NOT return ptime (design decision — avoids duplicating statx functionality).
  - **All tool patches updated**: cp, rsync, tar, KIO now call `syscall(469, dfd, path, &file_attr, 40, at_flags)`.
  - **glibc limitation eliminated**: file_setattr uses a struct pointer, not a fixed-size array. No wrapper conflicts.
  - **Tar bug fixed**: create.c statx read used AT_FDCWD (wrong with -C flag); changed to fd+AT_EMPTY_PATH.
  - **Patch stats**: v6 = 648 lines, 26 files (v5 was 717 lines, 28 files — smaller due to utimensat removal).
  - Wong confirmed: `__s64 sec + __u32 nsec` matching statx, FICLONE/exchange-range should copy ptime (v3 scope), Pali Rohar mask work orthogonal.
...
| **Standard API** | statx() to read, file_setattr() to write — modern size-versioned extensible struct |
...
|---|---|---|
| cp (coreutils) | Add statx PTIME read + file_setattr write | ~30 lines |
...
| tar | Add SCHILY.ptime PAX header (create + extract) | ~50 lines |
| btrfs-progs | Decode ptime in dump-tree | ~20 lines |

**Why tools don't auto-detect**: Tools hardcode which timestamps they copy (typically atime + mtime). Adding ptime requires each tool to: (1) read ptime via `statx(STATX_PTIME)` at offset 0xC0, (2) write ptime via `syscall(SYS_file_setattr, dfd, path, &file_attr, 40, flags)` with ptime fields at offset 24 in a 40-byte `struct file_attr`. The `file_setattr` syscall (469, merged Linux 6.17) uses a size-versioned struct — no glibc wrapper conflicts. For upstream tool adoption, once glibc ships `struct file_attr` headers, tools can use a standard function call instead of raw syscall.
...
| File copied (with patched tool) | Tool reads source ptime via statx, writes to dest via file_setattr. |
...
Same as mtime setting via notify_change():
...
|---|---|
| Sandoval's AT_UTIME_BTIME (2019) | ptime addresses the same need (settable creation date) but via file_setattr instead of utimensat. Avoids btime mutability dispute AND the glibc times[2] limitation that blocked Sandoval's approach. |
...
- `struct file_attr` VER1 extension with ptime fields for file_setattr()
...
### 4.3 file_setattr Extension (v6 — replaces utimensat)

**`include/uapi/linux/fs.h`** — extend `struct file_attr` with VER1:
```c
struct file_attr {
    __u64 fa_xflags;      /* xflags (VER0, offset 0) */
    __u32 fa_extsize;     /* extent size hint (VER0, offset 8) */
    __u32 fa_nextents;    /* number of extents (VER0, offset 12) */
    __u32 fa_projid;      /* project ID (VER0, offset 16) */
    __u32 fa_cowextsize;  /* CoW extent size (VER0, offset 20) */
    /* ptime extension — VER1 */
    __s64 fa_ptime_sec;   /* provenance time: seconds since epoch (offset 24) */
    __u32 fa_ptime_nsec;  /* provenance time: nanoseconds (offset 32) */
    __u32 fa_ptime_pad;   /* reserved, must be zero (offset 36) */
};

#define FILE_ATTR_SIZE_VER0  24
#define FILE_ATTR_SIZE_VER1  (FILE_ATTR_SIZE_VER0 + 16)  /* = 40 */
#define FILE_ATTR_SIZE_LATEST FILE_ATTR_SIZE_VER1
```

**`fs/file_attr.c`** — dual dispatch in `SYSCALL_DEFINE5(file_setattr, ...)`:
1. `copy_struct_from_user()` reads struct from userspace (size-versioned)
2. If `usize >= FILE_ATTR_SIZE_VER1`: validate `fa_ptime_pad == 0` and `fa_ptime_nsec < NSEC_PER_SEC`, set `ptime_valid = true`
3. If `ptime_valid`: construct `iattr` with `ATTR_PTIME | ATTR_PTIME_SET`, call `notify_change()` under `inode_lock()` — dispatches to per-FS `.setattr` (same path as utimensat v1, all 5 FS already handle ATTR_PTIME)
4. If any VER0 fields non-zero (xflags, extsize, projid, cowextsize): call `vfs_fileattr_set()` for existing flag handling
5. If neither ptime nor VER0 fields: return `-EINVAL`

**`include/linux/fileattr.h`** — extend `struct file_kattr`:
```c
struct file_kattr {
    /* ... existing fields ... */
    bool ptime_valid:1;
    struct timespec64 ptime;
};
```

**Backward compatibility**: Old userspace (VER0, 24 bytes) → new kernel: ptime fields zeroed by `copy_struct_from_user`, `ptime_valid = false`, only VER0 path executes. New userspace (VER1, 40 bytes) → old kernel: `copy_struct_from_user` checks trailing non-zero bytes, returns `-E2BIG` (userspace can fall back to VER0-only call).

**Replaces** (removed in v6): `AT_UTIME_PTIME` flag, `times[2]` extension in `do_utimes()`, `vfs_utimes()` flags parameter, 3 callers (btrfs/volumes.c, fs/init.c, init/initramfs.c).
...
NTFS (Date Created: 2019-06-15)
  ...
  → patched cp writes to Btrfs via file_setattr(ptime)
  → Btrfs ptime = 2019-06-15

Btrfs (ptime: 2019-06-15)
  → statx reads stx_ptime = 2019-06-15
  → patched cp writes to NTFS via file_setattr(ptime)
...
|---|---|---|---|---|
| 1 | vfs: add provenance_time (ptime) infrastructure | ATTR_PTIME (bit 19/20), STATX_PTIME, ia_ptime, setattr_prepare, file_attr.c (VER1 ptime extension + dual dispatch), stat.c, fileattr.h | 10 | ~80 |
...
| # | Deliverable |
|---|---|
| 17 | man-page updates: statx(2), file_setattr(2) — required for any UAPI change |
...
| 2 | Set ptime via file_setattr | ptime-set + ptime-stat | Reports set value; STATX_PTIME in mask |
...
| 17 | Set ptime to current time | file_setattr with fa_ptime_sec=0, fa_ptime_nsec=0 via ATTR_PTIME (no ATTR_PTIME_SET) | ptime set to current time by notify_change |
| 18 | VER0-only file_setattr (no ptime) | file_setattr with usize=24 (VER0) | ptime unchanged; only VER0 fields processed |
...
| 48 | file_setattr VER1 on old kernel (no ptime support) | -E2BIG (non-zero trailing bytes rejected by copy_struct_from_user) |
...
| glibc header availability | System headers may not yet define VER1 file_attr with ptime fields | Tools define struct locally or use raw buffer+memcpy; glibc update will add native support |
